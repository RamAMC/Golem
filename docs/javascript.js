...
document.addEventListener('DOMContentLoaded', () => {
  ...
  fileInput.addEventListener('change', async (event) => {
    ...
    if (files.length === 0) {
      initialPreviewContainer.classList.add('hidden');
      return;
    }
    ...
    // Orden estable por nombre (alfanumérico)
    const filesSorted = files.slice().sort((a, b) =>
      a.name.localeCompare(b.name, undefined, { numeric: true, sensitivity: 'base' })
    );

    // Helper para leer como DataURL
    ...
    const dataURLs = await Promise.all(filesSorted.map(readAsDataURL));

    // Render y estado final en orden
    initialPreviewContainer.innerHTML = '';
    images = new Array(filesSorted.length);
    ...
    dataURLs.forEach((src, i) => {
      ...
      images[i] = { file, src, name: file.name };
    });
  ...
  processBtn.addEventListener('click', () => {
    ...
    processedAspectRatio = aspectRatioSelect.value;
  ...
  function checkAspectRatioMatch() {
    if (!processedAspectRatio) return; // No ha procesado nada aún

    if (aspectRatioSelect.value !== processedAspectRatio) {
      downloadBtn.disabled = true;
      downloadBtn.classList.add('opacity-50', 'cursor-not-allowed');
      warningMessage.classList.remove('hidden');
      setTimeout(() => {
        warningMessage.classList.remove('opacity-0', 'translate-y-3');
        initialPreviewContainer.classList.add('mt-4');
      }, 10);
    } else {
      downloadBtn.disabled = false;
      downloadBtn.classList.remove('opacity-50', 'cursor-not-allowed');
      // Ocultar con animación
      warningMessage.classList.add('opacity-0', 'translate-y-3');
      setTimeout(() => {
        warningMessage.classList.add('hidden');
        initialPreviewContainer.classList.remove('mt-4');
